<template>
  <div class="semantic-library">
    ...
    <div class="content-card">
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          <el-select v-model="selectedCategory" placeholder="语义分类" style="width: 120px; margin-right: 8px" clearable>
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="description" label="语义描述" min-width="300" show-overflow-tooltip />
        <el-table-column prop="keywords" label="关键词" width="150">
          <template #default="{ row }">
            <el-tag v-for="keyword in row.keywords" :key="keyword" size="small" style="margin-right: 4px">
              {{ keyword }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog
      ...
      @close="resetForm"
    >
      <el-form :model="semanticForm" :rules="formRules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="语义分类" prop="category">
          <el-select v-model="semanticForm.category" placeholder="请选择语义分类" style="width: 100%">
            <el-option label="教学行为" value="teaching" />
            <el-option label="学习行为" value="learning" />
            <el-option label="情感表达" value="emotion" />
            <el-option label="知识概念" value="concept" />
      ...
      <div v-if="currentSemantic" class="semantic-detail">
        <div class="detail-section">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="语义名称">{{ currentSemantic.name }}</el-descriptions-item>
            ...
            <el-descriptions-item label="状态">
              <el-tag :type="currentSemantic.status === 'active' ? 'success' : 'info'">
                {{ currentSemantic.status === 'active' ? '启用' : '禁用' }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="使用次数">{{ currentSemantic.usageCount }}</el-descriptions-item>
            <el-descriptions-item label="创建时间">{{ currentSemantic.createTime }}</el-descriptions-item>
            <el-descriptions-item label="更新时间">{{ currentSemantic.updateTime }}</el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div class="detail-section">
          ...
          <div class="keywords-container">
            <el-tag v-for="keyword in currentSemantic.keywords" :key="keyword" size="large" style="margin: 4px">
              {{ keyword }}
            </el-tag>
          </div>
        </div>
        ...
        <div class="detail-section">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <div class="stat-card">
                <div class="stat-number">{{ currentSemantic.monthlyUsage || 0 }}</div>
                <div class="stat-label">本月使用</div>
              </div>
            </el-col>
            <el-col :span="8">
              <div class="stat-card">
                <div class="stat-number">{{ currentSemantic.weeklyUsage || 0 }}</div>
...
const handleDelete = async (row: Semantic) => {
  try {
    await ElMessageBox.confirm(`确定要删除语义 "${row.name}" 吗？`, '确认删除', {
      type: 'warning'
    })
...
const handleToggleStatus = async (row: Semantic) => {
  ...
  try {
    await ElMessageBox.confirm(`确定要${action}语义 "${row.name}" 吗？`, `确认${action}`, {
      type: 'warning'
    })
