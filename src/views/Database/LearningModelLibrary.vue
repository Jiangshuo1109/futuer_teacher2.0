<template>
  <div class="learning-model-library">
    ...
    <div class="content-card">
      ...
      <el-table :data="modelList" v-loading="loading" stripe>
        ...
        <el-table-column prop="accuracy" label="准确率" width="100">
          <template #default="{ row }">
            <span v-if="row.accuracy"
              >{{ (row.accuracy * 100).toFixed(1) }}%</span
            >
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="trainTime" label="训练时长" width="120">
          <template #default="{ row }">
            <span v-if="row.trainTime">{{
              formatTrainTime(row.trainTime)
            }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="学习率">
                  <el-input-number
                    v-model="modelForm.learningRate"
                    :min="0.0001"
                    :max="1"
                    :step="0.001"
                    :precision="4"
    ...
    <!-- 模型详情对话框 -->
    <el-dialog v-model="detailDialogVisible" title="模型详情" width="1000px">
      <div v-if="currentModel" class="model-detail">
        <div class="detail-header">
          <div class="model-info">
            <h2>{{ currentModel.name }}</h2>
            <div class="model-tags">
              <el-tag :type="getTypeTag(currentModel.type)" size="large">{{
                getTypeLabel(currentModel.type)
              }}</el-tag>
              <el-tag type="info" size="large" style="margin-left: 8px">{{
                currentModel.algorithm
              }}</el-tag>
              <el-tag
                :type="getStatusTag(currentModel.status)"
                size="large"
            ...
            <div class="model-stats">
              ...
              <div class="stat-item">
                ...
                <span v-if="currentModel.accuracy"
                  >{{ (currentModel.accuracy * 100).toFixed(1) }}%</span
                >
                <span v-else>-</span>
              </div>
              <div class="stat-item">
        ...
        <el-tabs>
            ...
            <div class="detail-section">
              ...
              <el-descriptions :column="2" border>
                ...
                <el-descriptions-item label="数据集大小">{{
                  formatDataSize(currentModel.datasetSize)
                }}</el-descriptions-item>
                <el-descriptions-item label="特征维度">{{
                  currentModel.featureDim || "-"
                }}</el-descriptions-item>
                <el-descriptions-item label="训练时长">{{
                  formatTrainTime(currentModel.trainTime) || "-"
                }}</el-descriptions-item>
              </el-descriptions>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="性能指标" name="performance">
            <div class="performance-charts">
              <div class="chart-item">
                ...
                <el-progress
                  :percentage="
                    currentModel.accuracy ? currentModel.accuracy * 100 : 0
                  "
                  :stroke-width="20"
                  :text-inside="true"
              ...
              <div class="chart-item">
                ...
                <el-progress
                  :percentage="
                    currentModel.precision ? currentModel.precision * 100 : 0
                  "
                  :stroke-width="20"
                  :text-inside="true"
                  color="#67c23a"
              ...
              <div class="chart-item">
                ...
                <el-progress
                  :percentage="
                    currentModel.recall ? currentModel.recall * 100 : 0
                  "
                  :stroke-width="20"
                  :text-inside="true"
                  color="#e6a23c"
              ...
              <div class="chart-item">
                ...
                <el-progress
                  :percentage="
                    currentModel.f1Score ? currentModel.f1Score * 100 : 0
                  "
                  :stroke-width="20"
                  :text-inside="true"
                  color="#f56c6c"
          ...
          <el-tab-pane label="部署信息" name="deployment">
            <div class="detail-section">
              ...
              <el-descriptions :column="2" border>
                <el-descriptions-item label="部署环境">{{
                  getDeployEnvLabel(currentModel.deployEnv) || "-"
                }}</el-descriptions-item>
                <el-descriptions-item label="服务端口">{{
                  currentModel.servicePort || "-"
                }}</el-descriptions-item>
            ...
            <div
              ...
              v-if="currentModel.resourceRequirements"
            >
              <h3>资源需求</h3>
              <div class="content-box">
...
const modelList = ref<LearningModel[]>([
  {
    ...
    version: "v1.2.0",
    description:
      "用于分析和预测学生在虚拟课堂中的行为模式，包括注意力集中度、参与度等指标",
    accuracy: 0.892,
    precision: 0.875,
    recall: 0.908,
    f1Score: 0.891,
  ...
  },
  {
    ...
    trainTime: 14400,
    trainParams:
      '{"optimizer": "adamw", "loss": "binary_crossentropy", "attention_heads": 8}',
  ...
  },
  {
    ...
    trainTime: null,
    trainParams:
      '{"optimizer": "adam", "loss": "sparse_categorical_crossentropy", "dropout": 0.3}',
    performanceNotes: "",
    deployEnv: "development",
    servicePort: 8083,
