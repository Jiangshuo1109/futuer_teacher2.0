      ...
      <div class="metrics-section">
        <div class="metrics-grid">
          <div
            ...
            :class="{ active: activeMetric === 'mandarin' }"
            @click="setActiveMetric('mandarin')"
          >
            ...
            <div
              class="metric-status"
              :class="getMetricStatusClass(metrics.mandarin.status)"
            >
          ...
          <div
            class="metric-card glass-card"
            :class="{ active: activeMetric === 'speed' }"
          ...
          >
            ...
            <div
              class="metric-status"
              :class="getMetricStatusClass(metrics.speed.status)"
            >
          ...
          >
            ...
            <div
              class="metric-status"
              :class="getMetricStatusClass(metrics.volume.status)"
            >
          ...
          <div
            class="metric-card glass-card"
            :class="{ active: activeMetric === 'keywords' }"
            @click="setActiveMetric('keywords')"
          >
            <div class="metric-icon">
              <el-icon><Key /></el-icon>
            </div>
            ...
            <div
              class="metric-status"
              :class="getMetricStatusClass(metrics.keywords.status)"
            >
          ...
          <div
            class="metric-card glass-card"
            :class="{ active: activeMetric === 'fillers' }"
            @click="setActiveMetric('fillers')"
          >
            <div class="metric-icon">
            ...
            <div
              class="metric-status"
              :class="getMetricStatusClass(metrics.fillers.status)"
            >
          ...
          <div
            class="metric-card glass-card"
            :class="{ active: activeMetric === 'questions' }"
            @click="setActiveMetric('questions')"
          >
            <div class="metric-icon">
            ...
            <div
              class="metric-status"
              :class="getMetricStatusClass(metrics.questions.status)"
            >
        ...
        <div class="chart-card glass-card">
          <div class="card-header">
            ...
            <div class="chart-controls">
              <el-button-group>
                <el-button
                  :type="timeRange === '5min' ? 'primary' : ''"
                  @click="setTimeRange('5min')"
                  >近5分钟</el-button
                >
                <el-button
                  :type="timeRange === '10min' ? 'primary' : ''"
                  @click="setTimeRange('10min')"
                  >近10分钟</el-button
                >
                <el-button
        ...
        <div class="details-card glass-card">
          <div class="card-header">
            ...
            <div class="details-summary">
              共找到
              <span class="highlight">{{ currentDetails.length }}</span> 项内容
            </div>
          </div>
              ...
              <div class="detail-content">
                ...
                <div class="detail-actions">
                  ...
                  <div class="detail-metrics" v-if="(item as any).metrics">
                    <span
                      v-for="metric in (item as any).metrics"
                      :key="metric.name"
                      class="metric-chip"
                    >
          ...
          <div class="report-content">
            ...
            <div class="report-dimensions">
              <div
                ...
                v-for="dimension in aiAnalysisReport.dimensions"
                :key="dimension.name"
              >
                ...
                <div class="dimension-suggestions" v-if="dimension.suggestions">
                  ...
                  <ul>
                    <li
                      v-for="suggestion in dimension.suggestions"
                      :key="suggestion"
                    >
                      {{ suggestion }}
                    </li>
...
const generateChartData = (metric: string) => {
  ...
  switch (metric) {
    ...
    case "volume":
      ...
      const volumeDataPoints = detailsData.volume.map((item) =>
        parseInt(item.metrics?.[0]?.value.replace("dB", "") || "0"),
      );
        ...
        xAxis: {
          ...
          axisLabel: {
            ...
            formatter: (value: number) =>
              `${Math.floor(value)}:${String(Math.floor((value % 1) * 60)).padStart(2, "0")}`,
          },
        },
        yAxis: {
          type: "value",
        ...
        series: [
          {
            ...
            itemStyle: {
              color: (params: any) => {
                ...
                return (
                  questionTypeColors[
                    params.data[2] as keyof typeof questionTypeColors
                  ] || "#409eff"
                );
              },
            },
  ...
  keywords: [
    {
      ...
      text: "学习、理解、应用",
      tags: [
        { text: "关键词", type: "primary" },
        { text: "李白", type: "success" },
      ],
      audioStart: 150,
      audioEnd: 155,
      keyword: "李白",
  ...
  questions: [
    {
      ...
      tags: [
        { text: "互动提问", type: "success" },
        { text: "开放性", type: "primary" },
      ],
      audioStart: 255,
      audioEnd: 260,
      questionType: "开放性问题",
...
const aiAnalysisReport = ref<AIAnalysisReport>({
  summary:
    "本次音频分析显示，教师整体语音表现良好。普通话发音标准，语速适中，音量控制得当。在课堂互动方面表现积极，提问频率合理。建议减少口头禅的使用，增强语言表达的流畅性。",
  dimensions: [
    {
      name: "普通话水平",
      analysis:
        "发音标准，语调自然，基本无明显方言影响。整体普通话水平达到二级甲等标准。",
      suggestions: ["继续保持标准发音", "注意语调的抑扬顿挫"],
    },
    {
      name: "语速控制",
      analysis:
        "平均语速200字/分钟，符合教学要求。在重点内容讲解时能够适当放慢语速。",
      suggestions: ["保持当前语速节奏", "在难点处可以再慢一些"],
    },
    {
      name: "音量控制",
      analysis:
        "音量适中，能够确保后排学生清楚听到。在强调重点时音量有所提高。",
      suggestions: ["继续保持音量控制", "可适当增加音量变化"],
    },
    {
      name: "课堂互动",
      analysis:
        "提问频率合理，平均每2分钟一个问题。能够给学生充分的思考和回答时间。",
  ...
  switch (activeMetric.value) {
    case "volume":
      ...
      if (dataIndex !== undefined && currentDetails.value[dataIndex]) {
        ...
        const detailElement = document.querySelector(
          `.detail-item:nth-child(${dataIndex + 1})`,
        );
        if (detailElement) {
          detailElement.scrollIntoView({ behavior: "smooth", block: "center" });
        }
        ElMessage.success(
          `跳转到时间点: ${currentDetails.value[dataIndex].time}`,
        );
      }
      break;
    ...
    case "questions":
      ...
      if (params.data && params.data[4]) {
        ...
        const detailIndex = currentDetails.value.findIndex(
          (item) => item.time === clickedTime,
        );
        if (detailIndex !== -1) {
          selectDetailItem(detailIndex);
          const detailElement = document.querySelector(
            `.detail-item:nth-child(${detailIndex + 1})`,
          );
          if (detailElement) {
...
const goToVideoAnalysis = () => {
  router.push(
    `/dashboard/teaching/task/${taskId}/student/${studentId}/video-analysis`,
  );
};
