<template>
  <div class="task-publish">
          ...
          <template #default="{ row }">
            <div class="action-buttons">
              <el-tooltip content="安排人员" placement="top" v-if="row.taskStatus === 'draft'">
                <el-button type="primary" size="small" circle @click="handleAssignPersonnel(row)">
              ...
              <el-tooltip content="查看" placement="top">
                <el-button type="info" size="small" circle @click="handleView(row)">
                  <el-icon><View /></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip content="修改" placement="top" v-if="row.taskStatus !== 'completed'">
    ...
    <!-- 创建任务对话框 -->
    <el-dialog v-model="showCreateDialog" title="创建教学任务" width="800px" :before-close="handleCloseDialog">
      <el-form :model="taskForm" :rules="taskRules" ref="taskFormRef" label-width="120px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="任务编号" prop="taskNumber">
              <el-input v-model="taskForm.taskNumber" disabled placeholder="系统自动生成" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="教学评价体系" prop="evaluationSystemId">
              <el-select v-model="taskForm.evaluationSystemId" placeholder="请选择评价体系" style="width: 100%">
                <el-option v-for="system in evaluationSystems" :key="system.id" :label="system.name" :value="system.id" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item label="学生模型选择" prop="studentModels">
          <div class="student-models">
            ...
            >
              ...
              <el-icon v-if="taskForm.studentModels.includes(model.id)" class="selected-icon"><Check /></el-icon>
            </div>
          </div>
        </el-form-item>

        <el-form-item label="特殊事件选择" prop="specialEvents">
          <el-select v-model="taskForm.specialEvents" multiple placeholder="请选择特殊事件" style="width: 100%">
            <el-option v-for="event in specialEventList" :key="event.id" :label="event.name" :value="event.id" />
          </el-select>
        </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="success" @click="handleCreateAndPublish">创建并发布</el-button>
        </span>
      </template>
    </el-dialog>
      ...
      <div class="assign-personnel-dialog">
        ...
        <div class="student-selection">
          <div class="selection-header">
            <el-checkbox
              :model-value="selectedStudents.length === filteredStudentList.length && filteredStudentList.length > 0"
              :indeterminate="selectedStudents.length > 0 && selectedStudents.length < filteredStudentList.length"
              @change="handleSelectAllStudents"
            >
              全选 ({{ selectedStudents.length }}/{{ filteredStudentList.length }})
            </el-checkbox>
          </div>
              ...
              >
                <el-checkbox :label="student.id">
                  <div class="student-info">
                    ...
                    <span class="student-class">{{ classList.find(c => c.id === student.classId)?.name }}</span>
                  </div>
                </el-checkbox>
              </div>
            </el-checkbox-group>
...
const taskRules: FormRules = {
  ...
  evaluationSystemId: [{ required: true, message: '请选择教学评价体系', trigger: 'change' }],
...
const filteredTableData = computed(() => {
  ...
  if (filterCourse.value) {
    data = data.filter(item => item.courseId.toString() === filterCourse.value)
  }
...
const filteredStudentList = computed(() => {
  ...
  if (selectedClass.value) {
    data = data.filter(student => student.classId.toString() === selectedClass.value)
  }
...
const handleDelete = async (row: Task) => {
  try {
    ...
    const index = tableData.value.findIndex(t => t.taskNumber === row.taskNumber)
    if (index > -1) {
...
const handleSelectAllStudents = () => {
  if (selectedStudents.value.length === filteredStudentList.value.length) {
    ...
    selectedStudents.value = filteredStudentList.value.map(student => student.id)
  }
}
...
const handleConfirmAssign = () => {
  ...
  ElMessage.success(`已为任务「${currentTask.value?.taskName}」安排人员：${selectedStudentNames}`)
...
const handleSaveDraft = async () => {
  try {
    ...
    const newTask: Task = {
      ...
      courseName: courseList.value.find(c => c.id === taskForm.value.courseId)?.name || '',
