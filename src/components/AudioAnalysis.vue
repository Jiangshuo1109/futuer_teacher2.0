          ...
          <div class="report-summary">
            ...
            <p>
              本次任务中，师范生在语音表达方面整体表现良好。普通话水平优秀率达到{{ 
                Math.round((audioData.mandarinStats.excellent / (audioData.mandarinStats.excellent + audioData.mandarinStats.good + audioData.mandarinStats.average + audioData.mandarinStats.poor)) * 100) 
              }}%，
              平均语速{{ audioData.speedStats.averageSpeed }}字/分钟，处于合理范围内。
              平均音量{{ audioData.volumeStats.averageVolume }}dB，音量控制适当。
            </p>
          </div>
          ...
          <div class="report-details">
            ...
            <div class="analysis-items">
              <div class="analysis-item">
                <h5>语音质量</h5>
                <p>{{ audioData.mandarinStats.excellent }}位师范生普通话达到优秀水平，发音标准，语调自然。建议对普通话水平一般和较差的{{ audioData.mandarinStats.average + audioData.mandarinStats.poor }}位师范生进行针对性训练。</p>
              </div>
              
              <div class="analysis-item">
                <h5>语速控制</h5>
                <p>平均语速{{ audioData.speedStats.averageSpeed }}字/分钟，符合教学语速标准。语速分布较为均匀，大部分师范生能够合理控制讲课节奏。</p>
              </div>
              
              <div class="analysis-item">
                <h5>教学用词</h5>
                <p>关键词使用丰富，平均每人使用{{ audioData.keywordStats.averagePerStudent }}个教学关键词。但口头禅使用较多，平均每人{{ audioData.fillerStats.averagePerStudent }}次，需要注意减少"嗯"、"然后"等口头禅。</p>
              </div>
              
              <div class="analysis-item">
                <h5>课堂互动</h5>
                <p>提问互动积极，平均每人提问{{ audioData.questionStats.averagePerStudent }}个问题。开放性问题使用较多，有利于启发学生思考。</p>
              </div>
            </div>
          </div>
  ...
  series: [
    {
      ...
      data: [
        { value: props.audioData.mandarinStats.excellent, name: '优秀', itemStyle: { color: '#67c23a' } },
        { value: props.audioData.mandarinStats.good, name: '良好', itemStyle: { color: '#409eff' } },
        { value: props.audioData.mandarinStats.average, name: '一般', itemStyle: { color: '#e6a23c' } },
        { value: props.audioData.mandarinStats.poor, name: '较差', itemStyle: { color: '#f56c6c' } }
      ]
    }
  ]
}))
